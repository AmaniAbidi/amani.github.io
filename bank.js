...
class Bank {
    ...
    closeAccount(number) {
        let accountIndex = this._accounts.findIndex((a) => a.getNumber() == number);
    ...
    accountReport() {
        return this._accounts.filter(a => a instanceof Account).map(a => a.toString()).join("\n");
    }
     ...
     */
    endOfMonth() {
        console.log(this._accounts.map(a => a.endOfMonth()));
    }
}